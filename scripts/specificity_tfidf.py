...
speaker_count = "twitter/speakers.csv"
...
intervals_pond = [round(x*0.01, 2) for x in range(17)]
###############################################################################

_thread.start_new_thread( keep_alive, tuple() )

print("Loading 32 language corpus...")
corpus = Corpus_32(leipzig_32_dir, speaker_count)
...
print("Most common words using weighted TF-IDF scores and specificity=0.1")
words_s002_pond = corpus.get_language_specific_words("gsw", 0.1, 400, True)
for x in words_s002_pond[:10]:
    print(x)
...
print([round(x*100, 2) for x in cov_gsw_pond])
print([round(x*100, 2) for x in cov_deu_pond])
